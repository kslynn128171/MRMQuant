...
% Final Update: Jul. 24, 2022
    ...
    idlist=find(contains(fileData,'id:')); % lines contains "id:"
    ...
    for i=(MRMnum+1):length(infoidx)
        uid=find(idlist>infoidx(i),1,'first');
        tempstr=fileData{idlist(uid)};
        cid=strfind(tempstr,':');
        chromdata.NonMRM=[chromdata.NonMRM;strtrim(tempstr((cid(1)+1):end))];
    end
end
