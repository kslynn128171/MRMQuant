...
function [concentration,flag]=find_conc_from_standard_curve(exp,compoundid,abundance)
    ...
    if ismember('p1',coeffnames(polpar)) % p1 is one of the coefficient
        %if strcmpi(type(polpar),'poly1') % the STD curve is a linear function
...
function [f,yreg]=standard_data_regression(x,y,keepid)
    ...
    elseif sum(useid)<3 % only two points are available, a linear line is used as standard curve
        ...
        if (std(y) < 0.05) && ~monotonic_increaing % check if the data is noisy and has small STD
            ...
            try
                f=fit(x(useid),y,'a*x^b','Lower',[0,0],'Upper',[Inf,3]); % use a power function to fit the data points
            catch
                f=fit(x(useid),y,'poly1','Lower',[0,0],'Upper',[Inf,0]); % use a linear curve to fit the data points            
            end
            yreg=feval(f,x);
        end 
    end
