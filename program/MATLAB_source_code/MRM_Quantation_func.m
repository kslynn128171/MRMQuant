function MRM_Quantation_func(para)
    ...
    bg_exp = uibuttongroup('Parent',l_panel, ...
        ...
        'BorderType','line',...
        'Fontsize',12,...
        'Position',[0.02 0.81 0.96 0.155],...
        'SelectionChangedFcn',@experiment_type_change,...
        'Title','Data Type',...
        'Visible','on');
    % radiobutton for data type of absolute quantitation with internal
    % standard
    uicontrol('Parent',bg_exp, ...
        ...
        'Value',para.abs_int);
    % radiobutton for data type of absolute quantitation with standard
    % curves
    rb_abs_stc=uicontrol('Parent',bg_exp, ...
        ...
        'Value',para.abs_stc);
    % radiobutton for data type of relative quantitation 
    uicontrol('Parent',bg_exp, ...
        'Units','normalized', ...
        'Fontsize',12, ...
        'BackgroundColor',bgcolor, ...
    ...
    uicontrol('Parent',pl_file, ...
        ...
        'Min',0,...
        'Max',10,...
        'Position',[0.02 0.01 0.96 0.76], ...
        'String',filelist, ...
        'Style','listbox', ...
        'Tag','list_file',...
        'CallBack',{@change_plot,'TIC'},...%@plot_TIC,...
        'UserData',filelist);
    %------------------------
    ...
    cnames={'Comp_Name','RT','RT_Tol','Parent_mz','Daughter_mz'};
...
function experiment_type_change(hobj,~)
    ...
    if ~is_saved && ~is_new_quant
        ...
        if strcmpi(answer,'Save and proceed')
            ...
            set(findobj('Tag','tbl_comp_list'),'Data',[]);
...
function filelist=browse_test_folder(~,~,hdlrec,hdldirinfo)
    ...
    if ~isempty(filelist) % legitimate files are found in the folder
        ...
        list1.Value=1;
        label=findobj('Tag','label1');
        label.String=sprintf('File list: %d files found.',length(filelist));
        nof=length(filelist);
        ...
        if ~isempty(get(findobj('tag','tbl_comp_list'),'Data')) % check if the information of expected compounds is given
            ...
            set(findobj('Tag','PB_modify'),'Enable','off');
        end
        show_quantitation_message('','',0,hdlpara);
        % clear existing plots
        cla(findobj('Tag','AX_heat_map'));
        cla(findobj('Tag','AX_TIC_plot'));
        ...
        warndlg('No MRM data in .txt format is found!','Warning',struct('WindowStyle','modal','Interpreter','tex'));
    end
end
%--------------------------------------------------------------
...
function change_method(~,~,target,hdlmeth,hdldirinfo)
    ...
    if file
        ...
        if strcmpi(ext,'.csv')
            ...
            lendelimter=length(commaloc);
            delimiter = ',';
        else
            spaceloc=strfind(str,' ');
            tabloc=strfind(str,'\t');
            lendelimter=max(length(spaceloc),length(tabloc));
        ...
        if para.abs_int % absolute quantitation with internal standard
            if lendelimter ~= 6
                errordlg({'File content is not recognizable !';'Need to include the following fields:';'Comp_Name, RT, RT_tol ,Parent_mz, Daughter_mz, IS, Conc'},'File Error',struct('WindowStyle','modal','Interpreter','none'));
                return;
            end
        elseif para.abs_stc % absolute quantitation with standard curve
            if ~((lendelimter >= 4) && (lendelimter <= 5))
                errordlg({'File content is not recognizable !';'Need to include the following fields:';'Comp_Name, RT, RT_tol, Parent_mz, Daughter_mz, (IS)'},'File Error',struct('WindowStyle','modal','Interpreter','none'));
                return;
            end
        else % relative quantitation
            if lendelimter ~= 4
                errordlg({'File content is not recognizable !';'Need to include the following fields:';'CompNams, RT, RT_tol, Parent_m/z, Daughter_m/z'},'File Error',struct('WindowStyle','modal','Interpreter','none'));
                return;
            end
        end
        dataStartLine = 2;
        extraColRule = 'ignore';
        if lendelimter == 4 % five columns of data
            varNames = {'Comp_Name','RT','RT_tol','Parent_mz','Daughter_mz'};
            varTypes = {'char','char','char','double','double'};
        elseif lendelimter == 5 % six columns of data
            varNames = {'Comp_Name','RT','RT_tol','Parent_mz','Daughter_mz','IS'};
            varTypes = {'char','char','char','double','double','char'};
        elseif lendelimter == 6 % seven columns of data
            varNames = {'Comp_Name','RT','RT_tol','Parent_mz','Daughter_mz','IS','Conc'};
        ...
        %hs = '<html><font size=5>'; %html start
        %he = '</font></html>'; %html end
        %varNames = cellfun(@(x)[hs x he],varNames,'uni',false); %with html
        opts = delimitedTextImportOptions('VariableNames',varNames,...
            ...
            'MissingRule','fill');
        opts = setvaropts(opts,'Comp_Name','FillValue','None');
        opts = setvaropts(opts,'RT','FillValue','-1');
        opts = setvaropts(opts,{'RT_tol'},'FillValue','-1');
        opts = setvaropts(opts,{'Parent_mz','Daughter_mz'},'FillValue',-1);
        if lendelimter >= 5
        ...
        for i=1:EICnum 
            ...
            if length(name) ~= length(method.rt{i})
                errordlg(['Compound number(',method.orig_name{i},') does not match with RT number (',...
                    data{i,2},')!'],'Method File Error',struct('WindowStyle','modal','Interpreter','tex'));
                return;
            end
        ...
        tblhdl.ColumnName=t.Properties.VariableNames; % set the table column names
        set(findobj('Tag','cb_normal'),'Value',1);
    else
...
function plot_EIC(objectHandle , ~, par1,par2)
    ...
    if (pbhdl.Value == 1) || (para.abs_stc == 1) 
        ...
        if isempty(axm) || is_change_mode
            ...
            % update the plots of neighboring compounds if the 'more>' is
            % pressed
            show_compound_in_nearby_files('','',fileid,compoundid,hdlrec,hdlmeth,'');
        end
        ...
        miny=min(rec{fileid}.data{EICid}(:,2)); % EIC min data height 
        maxstdy=0;
...
%         if flag 
%             warndlg('Quadratic regression failed for this compound! Use linear regression instead.','Warning',struct('WindowStyle','modal','Interpreter','none'));
%         end
    end
...
% ----------------------------------------------------
% generate a new window for an EIC and 
...
function fig=create_EIC_window(fromwhere,hdlrec,hdlmeth,hdlastc,hdlmtx,hdlpara)
    ...
    uicontrol('Parent',pl_indiv_EIC, ...
        ...
        'Enable','off',...
        'BackgroundColor',[0.75 0.75 0.75], ...
        'Callback',{@show_compound_in_nearby_files,0,0,hdlrec,hdlmeth,fig}, ...
    ...
    if strcmpi(fromwhere,'TIC') % called by clicking on a TIC plot
        ...
        mtx=hdlimg.CData;
        cmin=min(min(mtx)); %NaN is not included
        cmax=max(max(mtx)); %NaN is not included
        if cmin == cmax
            if cmin == 0
                cmax=1;
            else
...
function peak_quantitation_all(hobj, ~,target)
    ...
    stop_quant = contains(hobj.String,'Stop');%~any(matches(["New","Standard","Sample"],state)); % check if the user terminate the program in the middle of the quantitation process
...
function save_quantitation_result(~,~,hdlrec,hdlmeth,hdlastc,hdlpara)
    ...
    if file ~= 0 % if the user has specified a legitimate filename
        ...
        if ~get(findobj('Tag','rb_rel_quant'),'Value') % absolute quantitation
        ...
        set(findobj('Tag','PB_save_result'),'UserData',1,'Enable','off');
        pg_text.String='';
        pause(0.005);% better than drawnow;
    else
        % denote that the result has not been saved
        set(findobj('Tag','PB_save_result'),'UserData',0,'Enable','on');
    end
end
%--------------------------------------------------------------
% find the best match between expected compounds (method.rt) and
...
function close_check(~,~)
    if get(findobj('Tag','PB_save_result'),'UserData')==0
...
function draw_quantitation_heat_map(idx,hdlrec,hdlmeth,hdlpara,axhdl,imhdl)
    ...
    cmin=min(min(mtx)); %NaN is not included
    cmax=max(max(mtx)); %NaN is not included    
    if isnan(cmax) || isnan(cmin) || isinf(cmax) || isinf(cmin)
        hdl=errordlg('Some of the peak abundances are problematic!','Abundance Computation Error',struct('WindowStyle','modal','Interpreter','tex'));
        waitfor(hdl);
        return;
    end
    if cmin == cmax
...
function reassign(~,~,hdlrec,hdlmeth,hdlpara,hdlastc,hdlimg,fhdl,ahdl)
    ...
    % old_concentration=rec{fileid}.concentration{EICid}(peakid);
    % old_abundance=rec{fileid}.abundance{EICid}(peakid); 
    % old_conc_org=rec{fileid}.conc_org{EICid}(peakid);
    % old_quant_note=rec{fileid}.quant_note{EICid}(peakid);
    % old_is_compound=rec{fileid}.is_compound{EICid}(:,peakid);
    % old_is_peak=rec{fileid}.is_peak{EICid}(:,peakid);
    % old_decomp=rec{fileid}.decomp{EICid}(:,peakid);
    % old_bdp=rec{fileid}.bdp{EICid}(peakid,:);
    % old_coelute=rec{fileid}.coelute{EICid}(peakid,:);
    % old_is_defined_by_bound=rec{fileid}.is_defined_by_bound{EICid}(peakid);
    % % save original standard curve information
    % ax_stc=findobj('Tag','AX_std_curve');
    % chdl=ax_stc.Children;
    % oldstcx=cell(length(chdl),1);
    % oldstcy=cell(length(chdl),1);
    % for i=1:length(chdl)
    %     if isprop(chdl(i),'XData')
    %         oldstcx{i}=chdl(i).XData;
    %         oldstcy{i}=chdl(i).YData;
    %     end
    % end
    ...
    %     maxy=0;
    % end
    % miny=min(rec{fileid}.data{EICid}(:,2));
    % if isempty(maxy)
    %     maxy=miny+1;
    % elseif (maxy <= miny) 
    %     maxy=miny+1;
    ...
    % cb_show_detail=findobj('Tag','cb_show_detail');
    % if cb_show_detail.Value
    %     axis(ahdl,[minx maxx miny 1.05*maxy]);
    % end
    % cb_show_detail.UserData=[minx maxx miny 1.05*maxy];
    % % update the standard curve if the current data is a testing sample
    % if isempty(findobj('Tag','reg_result'))
    %     update_abundance_concentration_plot_in_EIC(hdlrec,hdlmeth,hdlpara,hdlastc,ax_stc,fileid,compoundid);
    % end
    ...
    if ~get(findobj('Tag','PB_sel_peak'),'UserData') % User clicks on 'Cancel', restore the original EIC plot    
        ...
        % rec{fileid}.abundance{EICid}(peakid)=old_abundance; 
        % rec{fileid}.conc_org{EICid}(peakid)=old_conc_org;
        % rec{fileid}.concentration{EICid}(peakid)=old_concentration;
        % rec{fileid}.is_compound{EICid}(:,peakid)=old_is_compound;
        % rec{fileid}.quant_note{EICid}(peakid)=old_quant_note;
        % rec{fileid}.is_peak{EICid}(:,peakid)=old_is_peak;
        % rec{fileid}.decomp{EICid}(:,peakid)=old_decomp;
        % rec{fileid}.bdp{EICid}(peakid,:)=old_bdp;
        % rec{fileid}.coelute{EICid}(peakid,:)=old_coelute;
        % rec{fileid}.is_defined_by_bound{EICid}(peakid)=old_is_defined_by_bound;
        % hdlrec.UserData = rec;
        ...
        % keepid=old_decomp>0;
        % if sum(keepid)>0
        %     set(hdl(6),'xdata',rec{fileid}.data{EICid}(keepid,1),'ydata',old_decomp(keepid),'visible','on');
        % else
        ...
        % for i=1:length(chdl)
        %     if isprop(chdl(i),'XData')
        %         chdl(i).XData=oldstcx{i};
        %         chdl(i).YData=oldstcy{i};
        %     end
        % end
        % % restore plot title
        % if para.abs_stc || para.abs_int
        %     title(ax_stc,['\fontsize{12}Concentration = {\color{red}',num2str(old_concentration,'%.2f'),'}']);
        % else
        %     title(ax_stc,['\fontsize{12}Rel. Abundance = {\color{red}',num2str(max(chdl(1).YData),'%.2f'),'%}']);
        % end
        ...
        % % update title
        % generate_title(fileid,EICid,peakid,filelist,ahdl,hdlrec,hdlmeth,hdlpara);
...
%--------------------------------------------------------------
% find compound concentration from a standard curve
%--------------------------------------------------------------
function [concentration,flag]=find_conc_from_standard_curve(exp,compoundid,abundance)
    ...
    if isinf(abundance) || isnan(abundance) 
        concentration=NaN;
        flag=true;
        return
    end
    if ismember('p1',coeffnames(polpar))
        if strcmpi(type(polpar),'poly2')
            if (abs(polpar.p1)<1e-6) && (abs(polpar.p2)<1e-6)
                concentration=NaN;
                flag=true;
                return
            end
        else
            if abs(polpar.p1)<1e-6
                concentration=NaN;
                flag=true;
                return
            end
    ...
    switch type(polpar)
        case 'poly2' % quadratic fitting
            polpar.p3=polpar.p3-abundance;
            try
                concentration=fzero(polpar,[0 2*exp.conc(end)]);
            catch
                concentration=interp1(ycoord,exp.conc,abundance,"spline","extrap");
            end
        case 'customnonlinear' % exponential fitting
            if ismember('a',coeffnames(polpar))
                if abs(polpar.a)<eps
                    concentration=nan;
                else
                    try
                        concentration=log((abundance-polpar.c)/polpar.a)/polpar.b;
                    catch
                        concentration=interp1(ycoord,exp.conc,abundance,"spline","extrap");
                    end
                end
            end
        otherwise % linear fitting
...
function rect_selection(~,~,hdlrec,hdlmeth,hdlpara,hdlastc,hdlimg,fhdl,ahdl)
    ...
    if (rect(3)<0.01)
        errordlg('The selected region is too small','Selection Error',struct('WindowStyle','modal','Interpreter','none'));
        ahdl.Toolbar.Visible = 'on';
        return;
    end
    ...
    update_plots_in_EIC_window(hdlrec,hdlmeth,hdlpara,hdlastc,ahdl,fileid,compoundid);
    % holdhdl=zeros(7,1);
    % holdhdl(1)=findobj('tag','lc_hdl1'); % expected compound location
    % holdhdl(2)=findobj('tag','lc_hdl2'); % original singal
    % holdhdl(3)=findobj('tag','lc_hdl3'); % smoothed signal
    % holdhdl(4)=findobj('tag','lc_hdl4'); % compound tip
    % holdhdl(5)=findobj('tag','lc_hdl5'); % compound peak
    % holdhdl(6)=findobj('tag','lc_hdl6'); % deconvoluted peak
    % holdhdl(7)=findobj('tag','lc_hdl7'); % background intensities
    % compid=rec{fileid}.is_compound{EICid}(:,peakid);
    % set(holdhdl(4),'XData',rec{fileid}.data{EICid}(compid,1),'YData',rec{fileid}.data{EICid}(compid,2));
    % % update the peak tip
    % rec{fileid}.is_peak{EICid}(:,peakid)=false;
    % rec{fileid}.is_peak{EICid}(pidx,peakid)=true;
    % ptid=rec{fileid}.is_peak{EICid}(:,peakid);
    % set(holdhdl(5),'XData',rec{fileid}.data{EICid}(ptid,1),'YData',rec{fileid}.data{EICid}(ptid,2));
    ...
    % % update the decomvoluted peak signal
    % rec{fileid}.decomp{EICid}(:,peakid)=zeros(size(rec{fileid}.decomp{EICid},1),1);
    % set(holdhdl(6),'XData',rec{fileid}.data{EICid}(:,1),'YData',rec{fileid}.decomp{EICid}(:,peakid),'Visible','off');
    % lstr={'Expected RT','Original Signal','Smoothed Signal','Detected Compound','Peak Tip Location','Background Intensity'};
    % legend(holdhdl([1:5,7]),lstr);
    % % update the abundance
    % tempy=rec{fileid}.data{EICid}(rec{fileid}.is_compound{EICid}(:,peakid),2)-rec{fileid}.bg_int{EICid}(rec{fileid}.is_compound{EICid}(:,peakid));
    % tempx=rec{fileid}.data{EICid}(rec{fileid}.is_compound{EICid}(:,peakid),1);
    % sumid=tempy>0;
    ...
    % rec{fileid}.quant_note{EICid}(peakid)=0; % assign the status as Successful Quantitation
    % % update the peak boundaries
    % peaks=rec{fileid}.data{EICid}(compid,1);
    % if isempty(peaks)
    %     lbound=rec{fileid}.data{EICid}(1,1);
    %     rbound=rec{fileid}.data{EICid}(end,1);
    %     difx=0;
    ...
    % maxx=min(rec{fileid}.data{EICid}(end,1),rbound+difx);
    % if ~isempty(compid)
    %     maxy=max(rec{fileid}.data{EICid}(compid,2));
    % else
    %     maxy=0;
    % end
    % miny=min(rec{fileid}.data{EICid}(:,2));
    ...
    %     cb_show_detail.UserData=[minx maxx miny 1.05*maxy];
    % end
    % For test samples, update the standard curve in the EIC window. 
    % For std samples, the update will be done in the select_EIC
    % function
    % if isempty(findobj('Tag','reg_result')) % For test samples only.  
    %     % recompute the concentration and update the standatd curve
    %     update_abundance_concentration_plot_in_EIC(hdlrec,hdlmeth,hdlpara,hdlastc,ax_stc,fileid,compoundid);
    % end
    % % save the update
    % hdlrec.UserData=rec;
    ...
    if ~get(findobj('Tag','PB_sel_region'),'UserData') % restore the original EIC plot    
        ...
        % hdlrec.UserData=rec;
        % set(holdhdl(4),'XData',rec{fileid}.data{EICid}(compid,1),'YData',rec{fileid}.data{EICid}(compid,2));
        % % restore the peak tip
        % ptid=rec{fileid}.is_peak{EICid}(:,peakid);
        % set(holdhdl(5),'XData',rec{fileid}.data{EICid}(ptid,1),'YData',rec{fileid}.data{EICid}(ptid,2));
        % tempv=rec{fileid}.decomp{EICid}(:,peakid);
        % keepid=tempv>0;
        % if sum(keepid)>0
        %     set(holdhdl(6),'Xdata',rec{fileid}.data{EICid}(keepid,1),'YData',tempv(keepid),'visible','on');
        % else
        %     set(holdhdl(6),'Xdata',rec{fileid}.data{EICid}(:,1),'YData',zeros(size(rec{fileid}.data{EICid}(:,1))),'visible','off');
        % end
        % % restore the deconvoluted peak and legend
        % if get(findobj('Tag','cb_show_legend'),'Value')
        %     if sum(rec{fileid}.decomp{EICid}(:,peakid))==0
        %         legend(holdhdl([1:5,7]),{'Expected RT','Original Signal','Smoothed Signal','Detected Compound','Peak Tip Location','Background Intensity'});
        %     else
        %         legend(holdhdl,{'Expected RT','Original Signal','Smoothed Signal','Detected Compound','Peak Tip Location','Deconvoluated Signal','Background Intensity'});
        %     end
        % else
        %     legend('hide');
        % end
        ...
        % end
        % if cb_show_detail.Value
        %     bounds=get(cb_show_detail,'UserData');
        %     axis(ahdl,bounds);
        % end
    else % commence the related updates
        % update the concentration and heatmap in the main window
...
function check_modify(~,eventdata)
    ...
    % if the modification is not in the first column, check the input
    % string
    if c == 2
...
function clear_existing_results()
    mainwin=findobj('Tag','MRM_Quant');
    set(mainwin,'WindowButtonMotionFcn',[],'WindowKeyPressFcn',[]);
    ...
    set(findobj('Tag','PB_save_result'),'UserData',[]);
    set(findobj('Tag','pl_comp'),'UserData',[]);
...
function change_EIC(hobj,~,direct,hdlmeth,hdlastc,hdlaxhm,hdlimg)
    ...
    % find number of sample files
    fhdl=findobj('Tag','list_file');
    %if strcmpi(get(findobj('Tag','PB_quant'),'UserData'),'New')
    if contains(lower(get(findobj('Tag','PB_quant'),'UserData')),'standard') % quantitation on standard compound data
...
function prepare_standard_curve_data(~, ~)
    ...
    pl_std_con=uipanel('Parent',stdwin, ...
        ...
        'FontSize',12,...
        'HighlightColor',[0,0,0],...
        'Position',[0.05 0.69 0.9 0.3], ...
        'Title','Standard Compounds and Their Concentrations',...
        'Tag','pl_std_con');
    % entry for file path 
    ...
    pl_std_rt=uipanel('Parent',stdwin, ...
        ...
        'HighlightColor',[0,0,0],...
        'FontSize',12,...
        'Position',[0.05 0.38 0.9 0.3], ...
        'Title','Standard Compound Retention Time',...
        'Tag','pl_std_rt',...
...
function filelist=browse_std_folder(~, ~)
    ...
    if newpath ~= 0
        ...
        pl_std_con=findobj('Tag','pl_std_con');
        ...
        method.fname=filelist;
        method.conc=con;
        pl_std_con.UserData=method; 
...
function compute_standard_curve(hdlrec,hdlmeth,hdlastc,hdlpara)
    ...
    for i=1:nop % compute the standard curve for each compound
        ...
        [f,~]=standard_data_regression(exp.conc,ycoord,keepid);  
    ...
    if exp.is_show_regression % if the show regression option is selected
        % generate a window to show regression curves and the corresponding
        % EICs
        ...
        if para.normal % perform batch effect correction
            batch_effect_correction_para(hdlrec,hdlpara);
...
function update_std_result(update_para,hdlrec,hdlmeth,hdlastc)
    ...
    for i=update_para.start_comp:update_para.end_comp
        ...
        [f,yreg]=standard_data_regression(exp.conc,ycoord,keepid);
        if isempty(f), return;end
        maxy=max(yreg);
        if isnan(maxy)
            maxy=Inf;
...
function show_heatmap_options(~,~,hdlrec,hdlmeth,hdlpara,hdlaxhm)
    ...
    if para.rel_quant || (strcmpi(get(findobj('tag','PB_quant'),'UserData'),'standard+') && para.abs_stc) 
        show_abundance=1;
    else
...
function take_log(hobj,~,hdlaxhm,hdlpara)
    ...
    if hobj.Value
        ...
        set(imhdl,'CData',mtx,'AlphaData',showalpha,'UserData',alpha);
        hdlaxhm.CLim = [min(min(mtx)),max(max(mtx))];
    end
    hdlaxm_mini=findobj('Tag','AX_miniature');
    imhdl_mini=findobj('Tag','im_heatmap_mini');
    if ~isempty(hdlaxm_mini)
...
function change_heatmap_abundent(~,~,hdlrec,hdlmeth,hdlpara,hdlaxhm)
    ...
    if ~isempty(heatmapoptwin)
        ...
        para=hdlpara.UserData;
        is_show_abund=para.heatmap_abund || para.rel_quant;
...
function adjust_background(~,~,hdlrec,hdlmeth,hdlpara,hdlastc,hdlimg,fhdl,ahdl)
    ...
    old_rec=temparary_save_EIC(rec,fileid,EICid,peakid,ahdl);
    % old_bg_int=rec{fileid}.bg_int{EICid};
    % old_abundance=rec{fileid}.abundance{EICid}(peakid);
    % old_conc_org=rec{fileid}.conc_org{EICid}(peakid);
    % old_concentration=rec{fileid}.concentration{EICid}(peakid);
    % old_is_defined_by_bound=rec{fileid}.is_defined_by_bound{EICid}(peakid);
    ...
    if ~get(findobj('Tag','PB_adj_bg'),'UserData') % restore the original EIC plot
        % restore all the peak info
        hdlrec.UserData = restore_EIC(rec,old_rec,fileid,EICid,peakid,para,ahdl);
        % rec{fileid}.bg_int{EICid}=old_bg_int;
        % rec{fileid}.abundance{EICid}(peakid)=old_abundance; 
        % rec{fileid}.conc_org{EICid}(peakid)=old_conc_org;
        % rec{fileid}.concentration{EICid}(peakid)=old_concentration;
        % rec{index}.is_defined_by_bound{EICid}(peakid)=old_is_defined_by_bound;
        % hdlrec.UserData=rec;
        % % restore the standard curve plot
        % for i=1:length(chdl)
        %     if isprop(chdl(i),'XData')
...
function update_EICs(~,~,update_para)
    ...
    if (update_para.end_comp > 0) || (update_para.end_file > 0)
        ...
        %is_QC=false; % check whether the modified files contain QC samples
        %is_QC=is_QC | rec{update_para.fileid}.batch_QC;
...
function show_more_compound(hobj,~,pl_cur_comp,pl_more_comp,hdlrec,hdlmeth,fhdl)
    if hobj.UserData == 0
        ...
        if isempty(findobj('Tag','comphdl11')) % no plot was drawn before
            ...
            % update the plots of the same compound in other files
            show_compound_in_nearby_files('','',fileid,compoundid,hdlrec,hdlmeth,fhdl);
        end
    else
...
% button is pressed
%--------------------------------------------------------------------------
function show_compound_in_nearby_files(hobj,~,fileid,compoundid,hdlrec,hdlmeth,fhdl)
...
function show_standard_curve_related_comps(hobj,~,hdlrec,hdlmeth,hdlastc)
    ...
    % compute the absolute compound id through slider position
    slhdl1=findobj('Tag','SL_StandCurve');
    compid=(1.0-slhdl1.Value)*(exp.nocomp-16)+stdcur_plotid;
...
% ------------------------------------------------------------------------
function [f,yreg]=standard_data_regression(x,y,keepid)
    % remove nan in y
    ...
    elseif sum(useid)<3 % only two points are available, a linear line is used as standard curve
        f=fit(x(useid),y,'poly1'); % use a linear curve to fit the data points
        yreg=feval(f,x);
    else
        % determine the regression form
        yincr=(y(2:end)-y(1:end-1))>=0;
        incrratio=sum(yincr)/length(yincr);
        monotonic=(incrratio>0.7);% || (incrratio<0.3);
        if (std(y) < 0.05) && ~monotonic % check if the data is noisy and has small STD
            f=cfit(fittype('a*x+b'),0,mean(y)); % a flat line with the average value
            yreg=repmat(mean(y),size(x));
        else
            f=fit(x(useid),y,'poly2');%,options); % use a quadratic curve to fit the data points
            % check if the curve is monotonic
            yreg=feval(f,x);
            maxy=max(yreg);
            miny=min(yreg);
            is_decressing=false;
            if yreg(1)==miny && yreg(end)==maxy && f.p1<0
                f=fit(x(useid),y,'poly1'); % use a linear regression
                if f.p1 < 0 
                    is_decressing=true;
                end
            elseif ~(yreg(1)==miny && yreg(end)==maxy) % if not increasing %&& ~(yreg(1)==maxy && yreg(end)==miny) %the curve is not monotonic
                try
                    [f,gof2]=fit(x(useid),y,'a*exp(b*x)+c','StartPoint',[1,1,0]); % use a exponential curve to fit the data points
                    if f.a<0
                        is_decressing=true;
                    end
                    if gof2.rsquare <= 0.95 % residual is high
                        f=fit(x(useid),y,'poly1'); % use a linear regression
                        if f.p1 < 0 
                            is_decressing=true;
                        end
            ...
            if is_decressing % its a decreasing function
                f=cfit(fittype('a*x+b'),0,mean(y)); % a flat line with the average value
                yreg=repmat(mean(y),size(x));
            else
                yreg=feval(f,x);
...
% ------------------------------------------------------------
function answer=batch_effect_correction_para(hdlrec,hdlpara)
    global bgcolor
    rec=hdlrec.UserData;
    answer='Yes';
...
function edit_batch(hobj,event)
    ...
    elseif event.Indices(2) == 2 % change the reference 
        if hobj.Data{event.Indices(1),1} % if the sample has been selected as a QC
            ...
            %hobj.Data{event.Indices(1),2}=false;
        end
    end
end
% ------------------------------------------------------------
...
function batch_effect_correction(hdlrec,hdlmeth,hdlastc,hdlpara,hdlmtx,range)
    ...
    norm_abund=repmat(median(cmtx(QCid,:,1),1,'omitnan'),filenum,1); % initialize as median abundance
    ...
    hdlrec.UserData=rec;
    % % check whether the abundances or the concentrations are used in the
    % % heatmap
    % if ~para.heatmap_abund && ~para.rel_quant
    %     mtx=cmtx(:,:,2);
    % else
    %     mtx=cmtx(:,:,1);
    % end
    % % get the heatmap handles
    % imhdl=findobj('Tag','im_heatmap');
    % alpha=imhdl.UserData;
    % axhdl=findobj('Tag','AX_TIC_plot');
    % % check whether to show the lower the transparency of the ambiguous compounds
    % if para.show_ambiguous_comp
    %     showalpha=alpha;
    % else
    %     showalpha=ones(size(alpha));
...
function detect_by_ref(~,~,hdlrec,hdlmeth,hdlpara,hdlastc,hdlmtx,fhdl)
    ...
    for i=1:nof % for each file
        if ismember(i,batch_ref_fileid),continue;end % the current file is a reference 
        for j=1:method.nocomp % for each compound
            ...
            if ~isempty(tip)
                if (abs(tip-tip_ref)<1e-2) && ~is_defined_by_bound && ~is_defined_by_bound_ref
                    continue;
                end 
            end
            ...
            if ~ is_peak_qualified % No qualified peak exist. Integrate signals in the same RT range of the reference peak
                ...
                if maxv >= para.sn_ratio*rec{i}.bg_int{EICid} % the signals are suficiently high
                    ...
                    %else
                    %    rec{i}.quant_note{EICid}(peakid)=4; % defected peak
                    end
                    % update the peak signals
...
function show_batch_effect_correction_result(~,~,hdlrec,hdlmeth,hdlpara,hdlmtx)
    ...
    if para.normal == 0
        warndlg('The batch effect correction option was not checked and thus no information is available!','No batch effect correction Information','modal');
        return; 
